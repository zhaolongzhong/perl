...
while(<SEQALL>){
	...
	$tmpid  = $id; # store the last seqid.
	$tmpseq = $seq; # store the last seq. 
	if($line =~ />/){
...
my $single_seq_path = $query_seq_dir."/".$tmpid.".fasta";
open(WRITE, ">$single_seq_path");
print WRITE ">$tmpid\n$tmpseq\n";
close(WRITE);
print LOG "$tmpid\n";
...
my $blast_out = "$output_dir"."/"."$tmpid".".out";
if(-e $blast_out){
	print $tmpid."existed\n";
	goto EXIST;
}
############run psiblast on the query single sequence on the database##########
my $psi_blast_command = "$psi_blast_exe -i $single_seq_path -d $db_blast_dir -j 3 >$output_dir"."/"."$tmpid".".out";	
print $psi_blast_command."\n";
print LOG "PSI-BLAST ".$psi_blast_command."\n";
`$psi_blast_command`;

############parse the psiblast results#########################################
    my $list_file = "$output_dir"."/"."$tmpid".".list";
my $parse_psiblast_command = "perl ./parse_blast_result.pl $output_dir"."/"."$tmpid".".out"." $list_file";
print $parse_psiblast_command."\n";
print LOG "PARSE ".$parse_psiblast_command."\n";
`$parse_psiblast_command`;
...
close(READLIST);
my $swiss_prot_part_output_file = $output_dir."/".$tmpid.".swiss_prot_DR";
if($ac_number){
	#############Parse the swiss-prot dataset##################
	my $ac_no = $ac_number;
	my $output_file = $swiss_prot_part_output_file;
